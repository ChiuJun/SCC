...
/*static int HasSideEffect(AstExpression expr) {
    ...
    if (expr->op == OP_ID && expr->ty->qualify & VOLATILE)
        return 1;
...
static void TranslateLabelStatement(AstStatement stmt) {
    ...
    if (labelStmt->label->refCount > 0) {
        /// if the label is not associated with a basic block,
        /// create a new basic block to associate with it
        if (labelStmt->label->corrBBlock == NULL) {
            labelStmt->label->corrBBlock = CreateBBlock();
        }
        StartBBlock(labelStmt->label->corrBBlock);
    }
...
static void TranslateGotoStatement(AstStatement stmt) {
    ...
    if (gotoStmt->label->corrBBlock == NULL) {
        gotoStmt->label->corrBBlock = CreateBBlock();
    }
    GenerateJump(gotoStmt->label->corrBBlock);
...
static void TranslateBreakStatement(AstStatement stmt) {
    ...
    if (brkStmt->target->astNodeKind == NK_SwitchStatement) {
...
static void TranslateSwitchBuckets(SwitchBucket *bucketArray, int left, int right,
    ...
    while (p) {
        i = p->expr->exprValue.i[0] - bucketArray[mid]->minVal;
        /**
    ...
    if (len == 1 && lhalfBB == rhalfBB) {
        GenerateBranch(choice->symbol_type, lhalfBB, JNE, choice, IntConstant(bucketArray[mid]->minVal));
    } else {
        GenerateBranch(choice->symbol_type, lhalfBB, JL, choice, IntConstant(bucketArray[mid]->minVal));

        StartBBlock(CreateBBlock());
        GenerateBranch(choice->symbol_type, rhalfBB, JG, choice, IntConstant(bucketArray[mid]->maxVal));
    }
    StartBBlock(CreateBBlock());

    if (len != 1) {
        index = CreateTemp(choice->symbol_type);
        GenerateAssign(choice->symbol_type, index, SUB, choice, IntConstant(bucketArray[mid]->minVal));
...
static void TranslateSwitchStatement(AstStatement stmt) {
     ...
     */
    if (sym->symbol_kind == SK_Constant) {
        Symbol t;
...
static void TranslateCompoundStatement(AstStatement stmt) {
    ...
    for (i = 0; i < GET_VECTOR_SIZE(ilocals); ++i) {
        ...
        v = GET_VECTOR_ITEM(ilocals, i);
        initd = AsVar(v)->data;

        if (HasPaddingSpace(AsVar(v))) {
            GenerateClear(v, v->symbol_type->size);
        }
        while (initd) {
            ty = initd->expr->ty;
            if (initd->expr->op == OP_STR) {
                String str = initd->expr->exprValue.p;
                src = AddString(ArrayOf(str->len + 1, ty->base_type), str, &initd->expr->astNodeCoord);
            } else {
...
static void TranslateStatement(AstStatement stmt) {
    (*StmtTrans[stmt->astNodeKind - NK_ExpressionStatement])(stmt);
}
...
void Translate(AstTranslationUnit transUnit) {
    ...
    while (p) {
        if (p->astNodeKind == NK_Function && ((AstFunction) p)->stmt) {
...

    {
        ...
        while (f != NULL) {
            if (f->storage_class == TK_STATIC && !f->defined && f->ref > 0) {
