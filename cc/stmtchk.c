...
#define PushStatement(v, stmt)   INSERT_VECTOR_ITEM(v, stmt)
#define PopStatement(v)          (--v->size)
#define TopStatement(v)          GET_VECTOR_BACK_ITEM(v)
...
static void AddCase(AstSwitchStatement swtchStmt, AstCaseStatement caseStmt) {
     ...
     */
    while (p) {
        diff = caseStmt->expr->exprValue.i[0] - p->expr->exprValue.i[0];
        if (diff < 0)
            break;
        ...
        if (diff > 0) {
            ...
            Error(&caseStmt->astNodeCoord, "Repeated constant in a switch statement");
            return;
        }
    }
...
static AstStatement CheckLabelStatement(AstStatement stmt) {
    ...
    if (labelStmt->label->isDefine) {
        ...
        */
        Error(&stmt->astNodeCoord, "Label name should be unique within function.");
    }
    labelStmt->label->isDefine = 1;
    labelStmt->label->labelCoord = stmt->astNodeCoord;
...
static AstStatement CheckCaseStatement(AstStatement stmt) {
    ...
    if (swtchStmt == NULL) {
        Error(&stmt->astNodeCoord, "A case label shall appear in a switch statement.");
        return stmt;
    }

    caseStmt->expr = CheckConstantExpression(caseStmt->expr);
    if (caseStmt->expr == NULL) {
        Error(&stmt->astNodeCoord, "The case value must be integer constant.");
        return stmt;
    }
...
static AstStatement CheckDefaultStatement(AstStatement stmt) {
    ...
    swtchStmt = (AstSwitchStatement) TopStatement(CURRENTF->swtches);
    if (swtchStmt == NULL) {
        Error(&stmt->astNodeCoord, "A default label shall appear in a switch statement.");
        return stmt;
    }
    if (swtchStmt->defStmt != NULL) {
        Error(&stmt->astNodeCoord, "There shall be only one default label in a switch statement.");
        return stmt;
    }
...
static AstStatement CheckIfStatement(AstStatement stmt) {
    ...
    if (!IsScalarType(ifStmt->expr->ty)) {
        Error(&stmt->astNodeCoord, "The expression in if statement shall be scalar type.");
    }
...
static AstStatement CheckSwitchStatement(AstStatement stmt) {
    ...
    if (!IsIntegerType(swtchStmt->expr->ty)) {
        Error(&stmt->astNodeCoord, "The expression in a switch statement shall be integer type.");
        swtchStmt->expr->ty = T(INT);
    }
    if (swtchStmt->expr->ty->category < INT) {
...
static AstStatement CheckLoopStatement(AstStatement stmt) {
    ...
    if (!IsScalarType(loopStmt->expr->ty)) {
        Error(&stmt->astNodeCoord, "The expression in do or while statement shall be scalar type.");
    }
...
static AstStatement CheckForStatement(AstStatement stmt) {
    ...
    if (forStmt->expr) {
        ...
        if (!IsScalarType(forStmt->expr->ty)) {
            Error(&stmt->astNodeCoord, "The second expression in for statement shall be scalar type.");
        }
    }
    // check expression-3
    if (forStmt->incrExpr) {
...
static AstStatement CheckGotoStatement(AstStatement stmt) {
    ...
    if (gotoStmt->id != NULL) {
        ...
        gotoStmt->label->labelCoord = gotoStmt->astNodeCoord;
        gotoStmt->label->refCount++;
    }
...
static AstStatement CheckBreakStatement(AstStatement stmt) {
    ...
    if (brkStmt->target == NULL) {
        Error(&stmt->astNodeCoord, "The break shall appear in a switch or loop");
    }
...
static AstStatement CheckContinueStatement(AstStatement stmt) {
    ...
    if (contStmt->target == NULL) {
        Error(&stmt->astNodeCoord, "The continue shall appear in a loop.");
    }
...
static AstStatement CheckReturnStatement(AstStatement stmt) {
    ...
    Type rty = FSYM->symbol_type->base_type;
    /**
    ...
    if (retStmt->expr) {
        ...
        if (rty->category == VOID) {
            Error(&stmt->astNodeCoord, "Void function should not return value");
            return stmt;
        }

        if (!CanAssign(rty, retStmt->expr)) {
            Error(&stmt->astNodeCoord, "Incompatible return value");
            return stmt;
        }
    ...
    if (rty->category != VOID) {
        Warning(&stmt->astNodeCoord, "The function should return a value.");
    }
    return stmt;
}
...
static AstStatement CheckStatement(AstStatement stmt) {
    return (*StmtCheckers[stmt->astNodeKind - NK_ExpressionStatement])(stmt);
}
