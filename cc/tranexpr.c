...
#define    GetBitFieldType(fld)    ((fld->field_type->category %2) ? T(UINT):T(INT) )
...
static Symbol TranslatePrimaryExpression(AstExpression expr) {
    if (expr->op == OP_CONST)
        return AddConstant(expr->ty, expr->exprValue);
    ...
    if (expr->op == OP_STR || expr->isArray || expr->isFunc) {
        ...
        //#endif
        return AddressOf(expr->exprValue.p);
    }

    return expr->exprValue.p;
}
...
static Symbol ReadBitField(Field fld, Symbol p) {
    int size = 8 * fld->field_type->size;
...
static Symbol WriteBitField(Field fld, Symbol dst, Symbol src) {
    ...
    if (src->symbol_kind == SK_Constant && src->val.i[0] > fmask) {
        Warning(dst->pcoord, " overflow in implicit constant conversion");
    }

    if (src->symbol_kind == SK_Constant && src->val.i[0] == 0) {
        p = Simplify(T(INT), BAND, dst, IntConstant(~mask));
    } else if (src->symbol_kind == SK_Constant && (src->val.i[0] & fmask) == fmask) {
        p = Simplify(T(INT), BOR, dst, IntConstant(mask));
    } else {
        if (src->symbol_kind == SK_Constant) {
    ...
    if (dst->symbol_kind == SK_Temp && AsVar(dst)->def->op == DEREF) {
        /**
             typedef struct{
                 int a;
                 int b:1;
        ...
        GenerateIndirectMove(fld->field_type, addr, p);
        dst = Deref(fld->field_type, addr);
    } else {
        GenerateMove(fld->field_type, dst, p);
    }
...
static Symbol TranslateArrayIndex(AstExpression expr) {
    ...
    do {
        if (p->kids[1]->op == OP_CONST) {
            coff += p->kids[1]->exprValue.i[0];
        } else if (voff == NULL) {
            voff = TranslateExpression(p->kids[1]);
        } else {
            voff = Simplify(voff->symbol_type, ADD, voff, TranslateExpression(p->kids[1]));
        }
        p = p->kids[0];
...
static Symbol TranslateFunctionCall(AstExpression expr) {
    ...
    while (arg) {
        ...
        INSERT_VECTOR_ITEM(args, ilarg);
        arg = (AstExpression) arg->next;
    }

    recv = NULL;
    if (expr->ty->category != VOID) {
...
static Symbol TranslateMemberAccess(AstExpression expr) {
     ...
     */
    fld = dst->val.p = expr->exprValue.p;
    /**
            typedef struct{
                int a;
                int b:1;
...
static Symbol TranslateIncrement(AstExpression expr) {
    ...
    if (expr->op == OP_POSTINC || expr->op == OP_POSTDEC) {
        ...
        if (casgn->kids[0]->isBitfield) {
            ret = ReadBitField(fld, p);
        } else if (p->symbol_kind != SK_Temp) {
...
static Symbol TranslateCast(Type ty, Type sty, Symbol src) {
    ...
    dcode = GetTypeCode(ty);
    scode = GetTypeCode(sty);
...
static Symbol TranslateAssignmentExpression(AstExpression expr) {
    ...
    if (expr->op != OP_ASSIGN) {
        ...
        expr->kids[0]->exprValue.p = expr->kids[0]->isBitfield ? ReadBitField(fld, dst) : dst;
    }
    src = TranslateExpression(expr->kids[1]);

    if (expr->kids[0]->isBitfield) {
        return WriteBitField(fld, dst, src);
    } else if (dst->symbol_kind == SK_Temp && AsVar(dst)->def->op == DEREF) {
        /**
...
static Symbol (*ExprTrans[])(AstExpression) =
        {
#define OPRINFO(op, prec, name, func, opcode) Translate##func##Expression,
...
AstExpression Not(AstExpression expr) {
    ...
    switch (expr->op) {
        ...
        CREATE_AST_NODE(t, Expression);
            t->astNodeCoord = expr->astNodeCoord;
            t->ty = T(INT);
            t->op = OP_NOT;
            t->kids[0] = expr;
...
void TranslateBranch(AstExpression expr, BBlock trueBB, BBlock falseBB) {
    ...
    switch (expr->op) {
        ...
        case OP_CONST:
            if (!(expr->exprValue.i[0] == 0 && expr->exprValue.i[1] == 0)) {
                GenerateJump(trueBB);
            }
            break;
