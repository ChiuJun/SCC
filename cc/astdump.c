...
static void DumpExpression(AstExpression expr, int pos) {
    ...
    switch (expr->op) {
        ...
        case OP_PTR_MEMBER:
            ...
            DumpExpression(expr->kids[0], pos);
            LeftAlign(ASTFile, pos);
            fprintf(ASTFile, "%s", ((Field) expr->exprValue.p)->field_id);
            fprintf(ASTFile, ")");
            break;
        ...
        case OP_ID:
            ...
            fprintf(ASTFile, "%s", ((Symbol) expr->exprValue.p)->name);
            break;

        case OP_STR: {
            String str = ((Symbol) expr->exprValue.p)->val.p;
            int i;

            if (expr->ty->category != CHAR)
                fprintf(ASTFile, "L");
            ...
            for (i = 0; i < str->len; ++i) {
                if (isprint(str->content[i]))
                    fprintf(ASTFile, "%c", str->content[i]);
                else
                    fprintf(ASTFile, "\\x%x", str->content[i]);
            }
        ...
        case OP_CONST: {
            int categ = expr->ty->category;

            if (categ == INT || categ == LONG || categ == LONGLONG) {
                fprintf(ASTFile, "%d", expr->exprValue.i[0]);
            } else if (categ == UINT || categ == ULONG || categ == ULONGLONG || categ == POINTER) {
                fprintf(ASTFile, "%u", expr->exprValue.i[0]);
            } else if (categ == FLOAT) {
                fprintf(ASTFile, "%g", expr->exprValue.f);
            } else {
                fprintf(ASTFile, "%g", expr->exprValue.d);
            }
        }
            break;
...
void DumpTranslationUnit(AstTranslationUnit transUnit) {
    ...
    ASTFile = CreateOutput((char *)Input.filename, ".ast");
