...
Symbol Simplify(Type ty, int opcode, Symbol src1, Symbol src2) {
    ...
    */
    if (src2 == NULL || (src2->symbol_kind != SK_Constant && opcode != SUB))
        goto add_value;
    ...
    switch (opcode) {
        case ADD:
            ...
            p1 = src1;
            c1 = 0;
            if (src1->symbol_kind == SK_Temp) {
                t = AsVar(src1);

                if (t->def->src2 && t->def->src2->symbol_kind == SK_Constant &&
        ...
        case SUB:
            // a - 0 = a
            if (src2->symbol_kind == SK_Constant && src2->val.i[0] == 0)
                return src1;
            ...
            } else if (src1->symbol_kind == SK_Constant) {
                p1 = NULL;
                c1 = src1->val.i[0];
            }
            p2 = src2;
            c2 = 0;
            if (src2->symbol_kind == SK_Temp) {
                t = AsVar(src2);
                if (t->def->src2 && t->def->src2->symbol_kind == SK_Constant &&
                    (t->def->op == ADD || t->def->op == SUB)) {
                    p2 = t->def->src1;
            ...
            } else if (src2->symbol_kind == SK_Constant) {
                p2 = NULL;
                c2 = src2->val.i[0];
            }
    ...
    while (inst != &bb->insth) {
        ...
        if ((inst->opcode == CALL || (inst->opcode >= EXTI1 && inst->opcode <= CVTF8U4)) &&
            ...
            */
            ...
            if (ninst->opds[0]->symbol_kind == SK_Temp)
                DefineTemp(ninst->opds[0], inst->opcode, (Symbol) inst, NULL);
        } else if (inst->opcode == MOV && inst->opds[1]->symbol_kind == SK_Temp) {
            /**
                t0 = 50;
                ....
                t0 = 60;
            ...
            */
            ...
            if (def && (def->op == MOV || def->op == CALL)) {
                ...
                if (inst->opds[0]->symbol_kind == SK_Temp) {
                ...
                bb->ninst--;
                   ...
                   inst->opds[2]->symbol_kind == SK_Constant) {
...
static void EliminateCode(BBlock bb) {
    ...
    while (inst != &bb->insth) {
        ...
        if (inst->opcode == CALL) {
            ...
            if (opds[0] && opds[0]->symbol_kind == SK_Temp && opds[0]->ref == 1 &&
                !IsRecordType(opds[0]->symbol_type)) {
                opds[0]->ref = 0;
                opds[0] = NULL;
            }
            ...
            /**
                ...
                OPCODE(MOV,     "=",                    Move)
             */
            if (opds[0]->symbol_kind == SK_Temp && opds[0]->ref == 1) {
                opds[0]->ref = 0;
                opds[1]->ref--;
