...
#define REPORT_OP_ERROR                                                 \
    Error(&expr->astNodeCoord, "Invalid operands to %s", OprNames[expr->op]);   \
    expr->ty = T(INT);                                                  \
    return expr;
...
static int CanModify(AstExpression expr) {
     ...
     */
    return (expr->isLvalue && !(expr->ty->qualify & CONST) &&
            (IsRecordType(expr->ty) ? !((RecordType) expr->ty)->hasConstField : 1));
}
...
static int IsNullConstant(AstExpression expr) {
    return expr->op == OP_CONST && expr->exprValue.i[0] == 0;
}
...
static AstExpression ScalePointerOffset(AstExpression offset, int scale) {
    ...
    val.i[1] = 0;
    val.i[0] = scale;
    expr->kids[1] = Constant(offset->astNodeCoord, offset->ty, val);
...
static AstExpression PointerDifference(AstExpression diff, int size) {
    ...
    val.i[1] = 0;
    val.i[0] = size;
    expr->kids[1] = Constant(diff->astNodeCoord, diff->ty, val);
...
static AstExpression CheckPrimaryExpression(AstExpression expr) {
    ...
    if (expr->op == OP_STR) {
        ...
        expr->op = OP_ID;
        expr->exprValue.p = AddString(expr->ty, expr->exprValue.p, &expr->astNodeCoord);
        expr->isLvalue = 1;

        return expr;
    }

    p = LookupID(expr->exprValue.p);
    if (p == NULL) {
        Error(&expr->astNodeCoord, "Undeclared identifier: %s", expr->exprValue.p);
        p = AddVariable(expr->exprValue.p, T(INT), Level == 0 ? 0 : TK_AUTO, &expr->astNodeCoord);
        expr->ty = T(INT);
        expr->isLvalue = 1;
    } else if (p->symbol_kind == SK_TypedefName) {
        /**
            typedef int a;
            int b;
            b = a;	------------ error
        */
        Error(&expr->astNodeCoord, "Typedef name cannot be used as variable");
        expr->ty = T(INT);
    } else if (p->symbol_kind == SK_EnumConstant) {
        /**
         ...
         */
        expr->op = OP_CONST;
        expr->exprValue = p->val;
        expr->ty = T(INT);
    } else {    // ID,  or function designatr
        expr->ty = p->symbol_type;
        expr->exprValue.p = p;
        expr->isInRegister = p->storage_class == TK_REGISTER;
        // an ID is a lvalue, while a function designator not.
        expr->isLvalue = expr->ty->category != FUNCTION;
    }
...
static AstExpression CheckArgument(FunctionType fty, AstExpression arg, int argNo, int *argFull) {
    ...
    if (!fty->sig->hasProto) {
        ...
        *argFull = 0;
        ...
        param = GET_VECTOR_ITEM(fty->sig->params, argNo - 1);
        if (!CanAssign(param->parameter_type, arg))
            goto err;

        if (param->parameter_type->category < INT)
            arg = Cast(T(INT), arg);
        else
            arg = Cast(param->parameter_type, arg);
    ...
    err:
    Error(&arg->astNodeCoord, "Incompatible argument");
    return arg;
}
...
static AstExpression CheckFunctionCall(AstExpression expr) {
    ...
    AstNode *tail;
    int argNo, argFull;
    if (expr->kids[0]->op == OP_ID && LookupID(expr->kids[0]->exprValue.p) == NULL) {
        expr->kids[0]->ty = DefaultFunctionType;
        expr->kids[0]->exprValue.p = AddFunction(expr->kids[0]->exprValue.p, DefaultFunctionType, TK_EXTERN,
                                                 &expr->astNodeCoord);
    } else {
    ...
    if (!(IsPtrType(ty) && IsFunctionType(ty->base_type))) {
        Error(&expr->astNodeCoord, "The left operand must be function or function pointer");
    ...
    *tail = NULL;
    ...
    if (argNo > VECTOR_SIZE(((FunctionType) ty)->sig->params)) {
        ...
        if (((FunctionType) ty)->sig->hasProto) {
            ...
            Warning(&expr->astNodeCoord, "Too few arguments");
        }
    }

    expr->ty = ty->base_type;
...
static AstExpression CheckMemberAccess(AstExpression expr) {
    ...
    if (expr->op == OP_MEMBER) {
        ...
        expr->isLvalue = expr->kids[0]->isLvalue;
    } else {
        /**
        For example:
            Type ty;
        ...
        expr->kids[0] = Adjust(expr->kids[0], 1);
        ty = expr->kids[0]->ty;
        if (!(IsPtrType(ty) && IsRecordType(ty->base_type))) {
            REPORT_OP_ERROR;
        }
        ty = ty->base_type;
        expr->isLvalue = 1;
    ...
    fld = LookupField(RemoveQualify(ty), expr->exprValue.p);
    if (fld == NULL) {
        Error(&expr->astNodeCoord, "struct or union member %s doesn't exsist", expr->exprValue.p);
        expr->ty = T(INT);
        return expr;
    }
    expr->ty = Qualify(ty->qualify, fld->field_type);
    expr->exprValue.p = fld;
    expr->isBitfield = fld->bits != 0;
    return expr;
}
...
static AstExpression CheckPostfixExpression(AstExpression expr) {
    switch (expr->op) {
        case OP_INDEX:
            ...
            if (IsIntegerType(expr->kids[0]->ty)) {
                SWAP_KIDS(expr);
            }
            if (IsObjectPtr(expr->kids[0]->ty) && IsIntegerType(expr->kids[1]->ty)) {
                expr->ty = expr->kids[0]->ty->base_type;
                /**
                    int arr[3][5];
                ...
                if (!expr->kids[0]->isArray && expr->ty->category != ARRAY) {
                    ...
                    deref->op = OP_DEREF;
                    deref->ty = expr->kids[0]->ty->base_type;
...
static AstExpression CheckTypeCast(AstExpression expr) {
    ...
    have scalar type.
     */
    if (!(BothScalarType(ty, expr->kids[1]->ty) || ty->category == VOID)) {
        Error(&expr->astNodeCoord, "Illegal type cast");
        return expr->kids[1];
    }
        ...
        case OP_ADDRESS:    // &a
            ...
            if (expr->kids[0]->op == OP_DEREF) {
                ...
                */
                ...
                expr->kids[0]->isLvalue = 0;
                return expr->kids[0];
            } else if (IsFunctionType(ty) ||
                       (expr->kids[0]->isLvalue && !expr->kids[0]->isBitfield && !expr->kids[0]->isInRegister)) {
                /**
                    see ansi.c.txt
        ...
        case OP_DEREF:    // *
            ...
            if (expr->kids[0]->op == OP_ADDRESS) {
                /**
                    ...
                    *&a ---------------->  a
                 */
                expr->kids[0]->kids[0]->ty = ty->base_type;
                return expr->kids[0]->kids[0];
            }
#if 0        // commented
                     ...
                     (ty->base_type->category == ARRAY || expr->kids[0]->kids[0]->isArray))
#endif
            {
                /**
                    int arr[3];
                ...
                expr->kids[0]->ty = ty->base_type;
                expr->kids[0]->isLvalue = 1;
                return expr->kids[0];
            }
            if (IsPtrType(ty)) {
                ...
                if (expr->ty->category == ARRAY || expr->kids[0]->isArray) {
                    union value val;
                    val.i[0] = val.i[1] = 0;
                    expr->kids[1] = Constant(expr->astNodeCoord, T(INT), val);
                    expr->op = OP_INDEX;
                }
                /**
                    int num = 3;
        ...
        case OP_SIZEOF:    // sizeof(a)
            /**
                 sizeof  unary-expression
                      sizeof (  type-name )
             */
            if (expr->kids[0]->astNodeKind == NK_Expression) {
                expr->kids[0] = CheckExpression(expr->kids[0]);
                // sizeof can't be applied to a bit-field
                if (expr->kids[0]->isBitfield)
                    goto err;
                ty = expr->kids[0]->ty;
            } else {
                ty = CheckTypeName((AstTypeName) expr->kids[0]);
...
static AstExpression CheckMultiplicativeOP(AstExpression expr) {
    ...
    if (expr->op == OP_MOD && BothIntegerType(expr->kids[0]->ty, expr->kids[1]->ty))
        goto ok;
...
static AstExpression CheckAddOP(AstExpression expr) {
    ...
    if (IsObjectPtr(ty2) && IsIntegerType(ty1)) {
        SWAP_KIDS(expr);
        ty1 = expr->kids[0]->ty;
        goto left_ptr;
    }

    if (IsObjectPtr(ty1) && IsIntegerType(ty2)) {
        left_ptr:
        /**
            int a = 3;
            int * ptr = &a;
         ...
         */
        expr->kids[1] = DoIntegerPromotion(expr->kids[1]);
        expr->kids[1] = ScalePointerOffset(expr->kids[1], ty1->base_type->size);
        expr->ty = ty1;
        return expr;
    }
...
static AstExpression CheckSubOP(AstExpression expr) {
    ...
    if (IsCompatiblePtr(ty1, ty2)) {
        expr->ty = T(INT);
        expr = PointerDifference(expr, ty1->base_type->size);
        return expr;
    }
...
static AstExpression CheckShiftOP(AstExpression expr) {
    if (BothIntegerType(expr->kids[0]->ty, expr->kids[1]->ty)) {
        expr->kids[0] = DoIntegerPromotion(expr->kids[0]);
        expr->kids[1] = DoIntegerPromotion(expr->kids[1]);
        expr->ty = expr->kids[0]->ty;
...
static AstExpression CheckRelationalOP(AstExpression expr) {
    ...
    if (IsObjectPtr(ty1) && IsObjectPtr(ty2) &&
        IsCompatibleType(RemoveQualify(ty1->base_type), RemoveQualify(ty2->base_type))) {
        return expr;
    }
    /**
        both operands are pointers to qualified or unqualified versions of
...
static AstExpression CheckEqualityOP(AstExpression expr) {
     ...
     */
    if (IsCompatiblePtr(ty1, ty2) ||
            (IsNotFunctionPtr(ty1) && IsVoidPtr(ty2)) ||
            (IsNotFunctionPtr(ty2) && IsVoidPtr(ty1)) ||
...
static AstExpression CheckAssignmentExpression(AstExpression expr) {
    ...
    */
    if (!CanModify(expr->kids[0])) {
        Error(&expr->astNodeCoord, "The left operand cannot be modified");
    }
    /**
    //	a += b  -------------->   a = a + b;
    ...
    if (expr->op != OP_ASSIGN) {
        ...
        lopr->astNodeCoord = expr->astNodeCoord;
    ...
    if (!CanAssign(ty, expr->kids[1])) {
        Error(&expr->astNodeCoord, "Wrong assignment");
    } else {
...
static AstExpression CheckConditionalExpression(AstExpression expr) {
    ...
    if (!IsScalarType(expr->kids[0]->ty)) {
        Error(&expr->astNodeCoord, "The first expression shall be scalar type.");
    }
    // the second operand
    ...
    if (BothArithType(ty1, ty2)) {
         ...
         */
        expr->ty = ty1;
    } else if (ty1->category == VOID &&
               ty2->category == VOID) {    // both operands have void type;  the result has void type.
        expr->ty = T(VOID);
    }
        /**
        ...
        /**
            ...
            compatible types;
         */
        qual = ty1->base_type->qualify | ty2->base_type->qualify;
        /**
         ...
         */
        expr->ty = PointerTo(
                Qualify(qual, CompositeType(RemoveQualify(ty1->base_type), RemoveQualify(ty2->base_type))));
    }
        /**
            if one operand is
         ...
         */
    else if ((IsNotFunctionPtr(ty1) && IsVoidPtr(ty2)) ||
            (IsNotFunctionPtr(ty2) && IsVoidPtr(ty1))) {
        /**
        ...
        Error(&expr->astNodeCoord, "invalid operand for ? operator.");
        expr->ty = T(INT);
    }
...
static AstExpression (*ExprCheckers[])(AstExpression) =
        {
#define OPRINFO(op, prec, name, func, opcode) Check##func##Expression,
...
static AstExpression CastExpression(Type ty, AstExpression expr) {
    ...
    if (expr->op == OP_CONST && ty->category != VOID)
        return FoldCast(ty, expr);

    CREATE_AST_NODE(cast, Expression);

    cast->astNodeCoord = expr->astNodeCoord;
    cast->op = OP_CAST;
    cast->ty = ty;
    cast->kids[0] = expr;
...
int CanAssign(Type lty, AstExpression expr) {
     ...
     */
    lty = RemoveQualify(lty);
    rty = RemoveQualify(rty);
    if (lty == rty) {
        return 1;
    }
    /**
     ...
     */
    if (IsCompatiblePtr(lty, rty) && ((lty->base_type->qualify & rty->base_type->qualify) == rty->base_type->qualify)) {
        return 1;
    }
    /**
        one operand is a pointer to an object or incomplete type and the
     ...
     */
    if (((IsNotFunctionPtr(lty) && IsVoidPtr(rty)) || (IsNotFunctionPtr(rty) && IsVoidPtr(lty))) &&
        ((lty->base_type->qualify & rty->base_type->qualify) == rty->base_type->qualify)) {
        return 1;
    }
...
#if 0
        Warning(&expr->astNodeCoord, "assignment from incompatible pointer type");
#endif
        return 1;
    }
    ...
    if (((IsPtrType(lty) && IsIntegerType(rty)) || (IsPtrType(rty) && IsIntegerType(lty))) &&
        (lty->size == rty->size)) {
        Warning(&expr->astNodeCoord, "conversion between pointer and integer without a cast");
        return 1;
    }
...
AstExpression Cast(Type ty, AstExpression expr) {
    ...
    if (scode < F4 && dcode < F4 && scode / 2 == dcode / 2) {
        ...
        int scateg = expr->ty->category;
        int dcateg = ty->category;
...
AstExpression Adjust(AstExpression expr, int rvalue) {
    ...
    int qual = 0;
    if (rvalue) {
        qual = expr->ty->qualify;
        expr->ty = RemoveQualify(expr->ty);
        expr->isLvalue = 0;
        //PRINT_CUR_ASTNODE(expr);
    }

    if (expr->ty->category == FUNCTION) {
        expr->ty = PointerTo(expr->ty);
        expr->isFunc = 1;
        //PRINT_I_AM_HERE();
    } else if (expr->ty->category == ARRAY) {
        expr->ty = PointerTo(Qualify(qual, expr->ty->base_type));
        /**
            "abc" = "123";		// illegal
            (1)
...
AstExpression DoIntegerPromotion(AstExpression expr) {
    return expr->ty->category < INT ? Cast(T(INT), expr) : expr;
}
...
AstExpression CheckConstantExpression(AstExpression expr) {
    ...
    if (!(expr->op == OP_CONST && IsIntegerType(expr->ty))) {
        return NULL;
    }
    return expr;
}
